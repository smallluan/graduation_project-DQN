        ...
        if self.current_player == 'player':
            if self.just_got_turn and action == 'pass':
                ...
                if self.last_played_triple:
                    # 必须出更大的三带一
                    valid_actions = self._find_triple_with_single(self.player_hand)
                    valid_actions = [act for act in valid_actions if int(act[:3][0]) > self.last_played]
                elif self.last_played_pair:
                    # 必须出更大的对子
                    pairs = self._find_pairs(self.player_hand)
                    valid_actions = [str(c) * 2 for c in pairs if c > self.last_played]
                else:
                    if self.last_played > 0:
                        # 上一轮出的是单牌，只能出更大的单牌
                        valid_single = [str(c) for c in self.player_hand if c > self.last_played]
                        valid_actions = valid_single
                    else:
                        # 初始状态或上一轮 pass，可以出单牌、对子或三带一
                        valid_single = [str(c) for c in self.player_hand if c > self.last_played]
                        valid_triple = [f"{t}{t}{t}{s}" for t in self._find_triples(self.player_hand) for s in
                                        self.player_hand if s != t]
                        valid_pair = [str(c) * 2 for c in self._find_pairs(self.player_hand) if c > self.last_played]
            ...
            valid_actions = []
            if self.last_played_triple:
                valid_actions = self._find_triple_with_single(self.ai_hand)
                valid_actions = [act for act in valid_actions if int(act[:3][0]) > self.last_played]
            elif self.last_played_pair:
                pairs = self._find_pairs(self.ai_hand)
                valid_actions = [str(c) * 2 for c in pairs if c > self.last_played]
            else:
                if self.last_played > 0:
                    valid_single = [str(c) for c in self.ai_hand if c > self.last_played]
                    valid_actions = valid_single
                else:
                    valid_single = [str(c) for c in self.ai_hand if c > self.last_played]
                    valid_triple = [f"{t}{t}{t}{s}" for t in self._find_triples(self.ai_hand) for s in self.ai_hand if
                                    s != t]
                    valid_pair = [str(c) * 2 for c in self._find_pairs(self.ai_hand) if c > self.last_played]
                    valid_actions = valid_single + valid_triple + valid_pair
...
# 训练 DQN
def train_dqn(env, num_episodes=num_episodes, gamma=gamma, epsilon=epsilon, learning_rate=learning_rate):
        ...
        while not done:
            if env.current_player == 'player':
                ...
                if env.last_played_triple:
                    valid_actions = env._find_triple_with_single(env.player_hand)
                    valid_actions = [act for act in valid_actions if int(act[:3][0]) > env.last_played]
                elif env.last_played_pair:
                    pairs = env._find_pairs(env.player_hand)
                    valid_actions = [str(c) * 2 for c in pairs if c > env.last_played]
                else:
                    if env.last_played > 0:
                        valid_single = [str(c) for c in env.player_hand if c > env.last_played]
                        valid_actions = valid_single
                    else:
                        valid_single = [str(c) for c in env.player_hand if c > env.last_played]
                        valid_triple = [f"{t}{t}{t}{s}" for t in env._find_triples(env.player_hand) for s in env.player_hand if s != t]
                        valid_pair = [str(c) * 2 for c in env._find_pairs(env.player_hand) if c > env.last_played]
                        valid_actions = valid_single + valid_triple + valid_pair

                # 如果刚拿到出牌权，禁止 pass
                if env.just_got_turn or env.last_action_was_pass or valid_actions:
                    valid_actions = [a for a in valid_actions if a != 'pass']
                ...
                if random.random() < epsilon:
                    valid_actions = []
                    if env.last_played_triple:
                        valid_actions = env._find_triple_with_single(env.ai_hand)
                        valid_actions = [act for act in valid_actions if int(act[:3][0]) > env.last_played]
                    elif env.last_played_pair:
                        pairs = env._find_pairs(env.ai_hand)
                        valid_actions = [str(c) * 2 for c in pairs if c > env.last_played]
                    else:
                        if env.last_played > 0:
                            valid_single = [str(c) for c in env.ai_hand if c > env.last_played]
                            valid_actions = valid_single
                        else:
                            valid_single = [str(c) for c in env.ai_hand if c > env.last_played]
                            valid_triple = [f"{t}{t}{t}{s}" for t in env._find_triples(env.ai_hand) for s in env.ai_hand if s != t]
                            valid_pair = [str(c) * 2 for c in env._find_pairs(env.ai_hand) if c > env.last_played]
                            valid_actions = valid_single + valid_triple + valid_pair

                    # 如果刚拿到出牌权，禁止 pass
                    if env.just_got_turn or env.last_action_was_pass:
                        valid_actions = [a for a in valid_actions if a != 'pass']
