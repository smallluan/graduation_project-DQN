...
reward_pass_with_valid = -30  # 有有效动作时 pass 的奖励，惩罚加重
reward_pass_without_valid = -5  # 无有效动作时 pass 的奖励
reward_single_card = 10  # 出单牌奖励，适当提高
reward_pair_card = 13  # 出对子奖励，适当提高
reward_triple_with_single = 15  # 出三带一奖励，适当提高
...
reward_ai_single_penalty = 20  # 出的单子比可出的单子大，需要惩罚，避免过早出掉大牌
reward_ai_pair_penalty = 20  # 出的单对子比可出的对子大，需要惩罚，避免过早出掉大牌
reward_ai_triple_penalty = 20  # 出的三带一比可出的三带一大，需要惩罚，避免过早出掉大牌
...
# 新增对战训练参数
online_training_steps = 10  # 每局游戏后训练次数
online_epsilon = 0.1  # 对战时的探索率
...
if __name__ == "__main__":
    ...
    criterion = nn.MSELoss()
    target_model = DQN(input_size)
    target_model.load_state_dict(loaded_model.state_dict())
    replay_buffer = ReplayBuffer(memory_capacity)
    ...
    while True:
        ...
        while not done:
            if current_player == 'player':
                ...
                if not valid_actions:
                    ...
                    if random.random() < online_epsilon:
                        ...
                        action = valid_actions[torch.argmax(valid_q).item()]

            # 执行动作并记录经验
            next_state, reward, done = env.step(action)
            next_state = torch.tensor(next_state, dtype=torch.float32).unsqueeze(0)  # 确保维度一致
            ...
            state = next_state.unsqueeze(0)  # 恢复批量维度
        ...
        # 每局游戏后进行在线训练
        for _ in range(online_training_steps):
            if len(replay_buffer) >= batch_size:
                batch_state, batch_action, batch_reward, batch_next_state, batch_done = replay_buffer.sample(batch_size)
                batch_state = torch.tensor(batch_state, dtype=torch.float32)
                batch_next_state = torch.tensor(batch_next_state, dtype=torch.float32)
                ...
                # 计算目标Q值
                with torch.no_grad():
                    next_q = target_model(batch_next_state)
                    max_next_q = torch.max(next_q, dim=1)[0]
                    target_q = batch_reward + (1 - batch_done) * initial_gamma * max_next_q.unsqueeze(1)

                # 计算当前Q值
                current_q = loaded_model(batch_state)
                action_indices = torch.tensor([ACTION_TO_INDEX[a] for a in batch_action], dtype=torch.long)
        ...
        # 更新目标网络
        train_counter += online_training_steps
        if train_counter % target_update_freq == 0:
            target_model.load_state_dict(loaded_model.state_dict())
            print("Target network updated")

        # 保存模型
        save_model(loaded_model, "models/model.pth")
        print("Model saved")
